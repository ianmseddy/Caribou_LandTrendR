...
var getSegmentCount = function(segData) {
  var mask = segData.arraySlice(0, 0, 1).arraySlice(1, 0, 1).neq(-9999)
                      .arrayProject([1]).arrayFlatten([['mask']]);
  return segData.arrayLength(1).select([0],['segCount']).updateMask(mask).toByte();
};
